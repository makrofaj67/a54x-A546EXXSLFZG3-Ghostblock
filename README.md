...
| **[purge-all.sh](file:///home/rakman/Projects/chrootundercve/purge-all.sh)** | Android (Root) | Complete revert. Stops all services, safely unmounts filesystems, deletes `/data/local/chroot`, wipes `/data/local/tmp`, cleans `/data/adb`, and restores SELinux to Enforcing. |
| **[adb-purge.sh](file:///home/rakman/Projects/chrootundercve/adb-purge.sh)** | PC (ADB) | One-click complete uninstallation from PC. Triggers `purge-all.sh` and optionally reboots the device into 100% factory locked state. |
